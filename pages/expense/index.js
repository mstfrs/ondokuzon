...
const Expenses = () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      console.log(error);
      messageApi.success("Check all fields")
    }
  form.resetFields()
  getTotal();
  };
  ...
  return (
    <>
    {contextHolder}
    <div className="container mx-auto mt-5 border bg-gradient-to-r from-blue-400 to-blue-600 rounded-2xl px-5 mb-4 pb-6">
      <h2 className="text-2xl font-bold ml-4 mt-4 text-center text-gray-700">Expenses</h2>
      <div className="flex justify-center items-center rounded-xl bg-gray-100 mx-6 py-4 mt-2">
        <h3 className="text-2xl font-medium">Total Expense : </h3>
        <h2 className="text-red-400 text-2xl font-semibold"> {total.toFixed(2)} {baseCurrency}</h2>
      </div>
      ...
      <div className="flex gap-10 mt-5 md:flex-row flex-col">
        ...
        <div className="flex-1 h-80  overflow-y-auto md:mr-4">
          {incomes.map((income) => (income.transactionType==="expense"&&
            <ListCard key={income.id} income={income} getIncomes={getIncomes} getTotal={getTotal} />
          ))}
        </div>
      </div>
    </div></>
